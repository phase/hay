...
class ClientSide is TCPConnectionNotify
  ...
  fun ref connecting(conn: TCPConnection ref, count: U32) =>
    _env.out.print("[Client] connecting: " + count.string())

  fun ref connected(conn: TCPConnection ref) =>
    try
      (let host, let service) = conn.remote_address().name()?
      _env.out.print("[Client] connected to " + host + ":" + service)
  ...
  fun ref connect_failed(conn: TCPConnection ref) =>
    _env.out.print("[Client] connect failed")

  fun ref received(conn: TCPConnection ref, data: Array[U8] iso,
    times: USize): Bool
  =>
    _env.out.write("[Client] Recieved data: ")
